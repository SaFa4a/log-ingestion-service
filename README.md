...
- **Test environment:** [hardware / cloud instance, Docker resource limits applied]
- **Dataset size:** [row count]
- **Batch size:** [logs per ingest request]
- **Sustained ingestion rate:** [logs/sec]
- **Aggregate query p95 latency:** [ms] (target: < 1s)
- **Query latency while ingestion is active:** [p50 / p95 / p99]
- **Resource usage at peak:** app CPU/RAM, Postgres CPU/RAM
- **Bottlenecks discovered:** [e.g. connection pool saturation, WAL checkpoint stalls]
- **Optimizations applied:** [e.g. unnest-based bulk insert, pool sizing, work_mem tuning]
